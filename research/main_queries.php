...
try {
    ...
    if (isset($_POST['prof']) || isset($_POST['domain']) || isset($_POST['year']) || isset($_POST['paper'])) {
        ...
        if (isset($_POST['year']) && strlen($_POST['year'])) {
            $r = $conn->query("SELECT * FROM {$myDB}.research_data WHERE ryear = {$_POST['year']}");
            while ($rowr = $r->fetch(PDO::FETCH_ASSOC)) {
                if (!(isset($_POST['is_mfsdsai']) && $_POST['is_mfsdsai'] == "yes" && $rowr['ris_mfdsai'] == 0)) {
                    $pr = $conn->query("SELECT * FROM {$myDB}.relation_pid_to_rid WHERE rid = {$rowr['rid']}");
        ...
        if (isset($_POST['paper'])) {
            $r = $conn->query("SELECT * FROM {$myDB}.research_data");
            while ($rowr = $r->fetch(PDO::FETCH_ASSOC)) {
        ...
        if (isset($_POST['domain'])) {
            $did = array();
            $d = $conn->query("SELECT * FROM {$myDB}.domain_data");
            ...
            for ($i = 0; $i < sizeof($did); $i++) {
                $dr = $conn->query("SELECT * FROM {$myDB}.research_paper_domain_label");
                while ($rowdr = $dr->fetch(PDO::FETCH_ASSOC)) {
                    $label = $rowdr['rdomain_label'];
                    if ($label[$did[$i] - 1] == '1') {
                        if (isset($_POST['is_mfsdsai']) && $_POST['is_mfsdsai'] == "yes") {
                            $r = $conn->query("SELECT * FROM {$myDB}.research_data WHERE rid = {$rowdr['rid']} AND ris_mfdsai = 1");
                            while ($rowr = $r->fetch(PDO::FETCH_ASSOC)) {
       ...
       $interesection = array_unique($interesection);
        ...
        if (!empty($interesection) && strlen($_POST['prof']) > 0) {
            ...
            while ($rowp = $p->fetch(PDO::FETCH_ASSOC)) {
                if (strlen($_POST['prof']) > 0 && str_contains(strtolower($rowp['pname']), $_POST['prof'])) {
                    ...
                    while ($rowpr = $pr->fetch(PDO::FETCH_ASSOC)) {
                        if (isset($_POST['is_mfsdsai']) && $_POST['is_mfsdsai'] == "yes") {
                            $r = $conn->query("SELECT * FROM {$myDB}.research_data WHERE rid = {$rowpr['rid']} AND ris_mfdsai = 1");
                            while ($rowr = $r->fetch(PDO::FETCH_ASSOC)) {
            ...
            foreach ($interesection as $key) {
                $pr = $conn->query("SELECT * FROM {$myDB}.relation_pid_to_rid WHERE rid = {$key}");
                while ($rowpr = $pr->fetch(PDO::FETCH_ASSOC)) {
        ...
        for ($i = 0; $i < sizeof($ans); $i++) {
            $tupple = array();
            $p = $conn->query("SELECT * FROM {$myDB}.prof_data WHERE pid = {$ans[$i][0]}");
            while ($rowp = $p->fetch(PDO::FETCH_ASSOC)) {
                array_push($tupple, $rowp['pname'], $rowp['pwebsite']);
            }
            $r = $conn->query("SELECT * FROM {$myDB}.research_data WHERE rid = {$ans[$i][1]}");
            while ($rowr = $r->fetch(PDO::FETCH_ASSOC)) {
                $domain = "";
                $did = array();
                $dr = $conn->query("SELECT * FROM {$myDB}.research_paper_domain_label WHERE rid = {$rowr['rid']}");
                while ($rowdr = $dr->fetch(PDO::FETCH_ASSOC)) {
                    $label = $rowdr['rdomain_label'];
                ...
                foreach ($did as $key) {
                    $d = $conn->query("SELECT * FROM {$myDB}.domain_data WHERE did = {$key}");
                    while ($rowd = $d->fetch(PDO::FETCH_ASSOC)) {
                        $domain .= ($rowd['dname'] . ', ');
                    }
                }
        ...
        foreach ($counts_pid as $key => $value) {
            $p = $conn->query("SELECT * FROM {$myDB}.prof_data WHERE pid = {$key}");
            while ($rowp = $p->fetch(PDO::FETCH_ASSOC)) {
