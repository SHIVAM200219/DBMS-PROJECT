<div class="text-light text-center" style="width:90%; margin:auto;">
    ...
    try {
        if (isset($_POST['prof']) && strlen($_POST['prof']) > 0) {
            ...
            for ($i = 0; $i < sizeof($pid_for_name); $i++) {
                ...
                $c = $conn->query("SELECT * FROM {$myDB}.citations_year");
    ...
    ?>
            ...
            for ($i = 0; $i < sizeof($pid_for_name); $i++) {
                ...
                $sql = "SELECT citation_year AS label, citation_count AS y FROM {$myDB}.citations_year WHERE pid = $pid_for_name[$i]";
                $result = $conn->query($sql);
                $dataPoints = $result->fetchAll(PDO::FETCH_ASSOC);


                $sql2 = "SELECT pname FROM {$myDB}.prof_data WHERE pid = $pid_for_name[$i]";
            ...
            for ($j = 0; $j < $len_p; $j++) {
                ...
                for ($i = 1; $i < 38; $i++) {
                    ...
                    FROM {$myDB}.research_paper_domain_label R , {$myDB}.domain_data D, {$myDB}.research_data RD , {$myDB}.prof_data P, {$myDB}.relation_pid_to_rid RP
