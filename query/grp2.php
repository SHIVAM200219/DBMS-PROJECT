  ...
  try {
    ...
    if (isset($_POST['year']) && strlen($_POST['year']) > 0) {
      ...
      for ($i = 1; $i < 38; $i++) {
        $stmt = $conn->query("SELECT dname, SUM(SUBSTRING(rdomain_label,$i,1)) AS paper_count
          FROM {$myDB}.research_paper_domain_label R , {$myDB}.domain_data D, {$myDB}.research_data RD
      ...
      echo "<div class =\"m-2 bg-dark rounded p-3 \" style = \"max-width : 90%; box-sizing:border-box;\">";
      echo "<h5 class = \" text-white text-center\">" . "PUBLISHERS IN THE YEAR " . $year . " are: </h5><br>";
      $stmt = $conn->query("SELECT DISTINCT rpublisher FROM {$myDB}.research_data RD WHERE RD.ryear = $year");
    ...
    if (isset($_POST['domain']) && strlen($_POST['domain']) > 0) {
      $domains = array();
      $stmt = $conn->query("SELECT DISTINCT dname FROM {$myDB}.domain_data ");
      while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
      ...
      $stmt = $conn->query("SELECT pname, SUM(SUBSTRING(rdomain_label,$did,1)) AS paper_count
        FROM {$myDB}.research_paper_domain_label R , {$myDB}.domain_data D, {$myDB}.prof_data P , {$myDB}.relation_pid_to_rid RP
      ...
      $stmt = $conn->query("SELECT DISTINCT rpublisher FROM {$myDB}.research_data RD , {$myDB}.domain_data D,{$myDB}.research_paper_domain_label R WHERE R.rid=RD.rid AND SUBSTRING(rdomain_label,$did,1) = 1");
      echo "<p class =\"text-center text-white\">";
      while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        if ($row['rpublisher'] != 'N/G') {
          echo $row['rpublisher'];
